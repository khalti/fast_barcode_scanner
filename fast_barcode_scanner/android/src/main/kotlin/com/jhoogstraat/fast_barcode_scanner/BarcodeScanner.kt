...
class BarcodeScanner(private val flutterTextureEntry: TextureRegistry.SurfaceTextureEntry, private val listener: (List<Barcode>) -> Unit) :
    RequestPermissionsResultListener, ActivityResultListener
{
    ...
    fun initialize(args: HashMap<String, Any>, result: Result)
    {
        ...
        if (activity == null) return ScannerError.ActivityNotConnected().throwFlutterError(result)
        ...
        val options = BarcodeScannerOptions.Builder().setBarcodeFormats(0, *scannerConfiguration.formats).build()
    ...
    {
        if (!isInitialized) return ScannerError.NotInitialized().throwFlutterError(result)
        else if (cameraProvider.isBound(preview)) return ScannerError.AlreadyRunning().throwFlutterError(result)
    ...
    {
        if (!isInitialized) return ScannerError.NotInitialized().throwFlutterError(result)
        else if (!cameraProvider.isBound(imageAnalysis)) return ScannerError.NotRunning().throwFlutterError(result)
    ...
    {
        if (!isInitialized) return ScannerError.NotInitialized().throwFlutterError(result)
        else if (!cameraProvider.isBound(preview)) return ScannerError.NotRunning().throwFlutterError(result)
    ...
    fun changeConfiguration(args: HashMap<String, Any>, result: Result)
    {
        ...
        try
        {
            val formats = if (args.containsKey("types")) (args["types"] as ArrayList<String>).map {
                barcodeFormatMap[it] ?: throw ScannerError.InvalidCodeType(it)
            ...
            val linearZoom =
                    if (args.containsKey("linearZoom")) (args["linearZoom"] as Double?)?.toFloat() else scannerConfiguration.linearZoom
            val exposureCompensationIndex =
                    if (args.containsKey("exposureCompensationIndex")) args["exposureCompensationIndex"] as Int? else scannerConfiguration.exposureCompensationIndex
    ...
    private fun bindCameraUseCases(result: Result? = null)
    {
        ...
        cameraSelector = CameraSelector.Builder().requireLensFacing(
                    if (scannerConfiguration.position == CameraPosition.back) CameraSelector.LENS_FACING_BACK
                    else CameraSelector.LENS_FACING_FRONT
                                                                   ).build()

        // TODO: Handle rotation properly
        preview = Preview.Builder().setTargetRotation(Surface.ROTATION_0).setTargetResolution(scannerConfiguration.resolution.portrait())
                .build()

        imageAnalysis = ImageAnalysis.Builder().setTargetRotation(Surface.ROTATION_0)
        ...
        when (source)
        {
            is List<*> -> barcodeDetector.analyze(
                InputImage.fromBitmap(
                    BitmapFactory.decodeByteArray(source[0] as ByteArray, 0, (source[0] as ByteArray).size), source[1] as Int
                                     )
            ...
            Activity.RESULT_OK ->
            {
                try
                {
                    barcodeDetector.analyze(activity!!.applicationContext, data?.data!!)
                            .addOnSuccessListener { sendBarcodes(it, pendingImageAnalysisResult!!) }.addOnFailureListener {
    ...
    private fun sendCameraDetails(result: Result)
    {
        ...
        previewConfiguration =
                PreviewConfiguration(flutterTextureEntry.id(), 0, previewRes.height, previewRes.width, analysis = analysisRes.toString())
